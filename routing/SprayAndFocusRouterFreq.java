 ...
 * 
 * @author PJ Dillon, University of Pittsburgh Modified by Raymond A.D.P.,
 *         Sanata Dharma University of Yogyakarta, Indonesia
 */
public class SprayAndFocusRouterFreq extends DecisionEngineRouter {
	 ...
	 * message copy = menentukan perbedaan nilai waktu yang dibutuhkan untuk
	...
	public SprayAndFocusRouterFreq(Settings s) {
		super(s);
	 ...
	 */
	public SprayAndFocusRouterFreq(SprayAndFocusRouterFreq r) {
		super(r);
	...
	public MessageRouter replicate() {
		return new SprayAndFocusRouterFreq(this);
	}
	...
	public void changedConnection(Connection con) {
		...
		if (con.isUp()) {
			...
			// menghapus neighbor encounter pada peer
		 ...
		 * duplicate the FreqEncounters Map and attach it to a message.se
		 */
		// menambahkan informasi dar objek FreqEncounter kedalam property pesan
		int msgSize = FreqEncounters.size() * 64
	...
	public boolean createNewMessage(Message m) {
		...
		// MSG_COUNT_PROP: properti untuk sisa salinan pesan yang tersisa
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		if (isDeliveredMessage(m) && peerEncounters != null) {
			// double distTo =
			// getHost().getLocation().distance(from.getLocation());
...

	{
		...
		// if (msg == null) { // message has been dropped from the buffer
		// after..
			...
			if (nrofCopies > 1) {
				...
				for (Connection c : getConnections()) {
					...
					if (peerEncounters != null
						...
						peerLastFreq = neighborEncounters.get(peer).get(dest)
								.getFreq();
					}
