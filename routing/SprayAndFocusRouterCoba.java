 ...
 */
public class SprayAndFocusRouterCoba extends ActiveRouter 
{
	/** SprayAndFocus router's settings name space ({@value})*/ 
	public static final String SPRAYANDFOCUS_NS = "SprayAndFocusRouterCoba";
	...
	public static final String MSG_COUNT_PROP = "SprayAndFocusCoba.copies";
	/** Message property key for summary vector messages exchanged between direct peers */
	public static final String SUMMARY_XCHG_PROP = "SprayAndFocusCoba.protoXchg";
	...
	protected Map<DTNHost, Double> Durasi;
	protected Map<DTNHost, List<Duration>> connHistory;
	protected Map<DTNHost, EncounterInfo> recentDurasi;
	...
	public SprayAndFocusRouterCoba(Settings s)
	{
		super(s);
		...
		Durasi = new HashMap<DTNHost, Double>();
		startTimestamps = new HashMap<DTNHost, Double>();
		//recentEncounters = new HashMap<DTNHost, EncounterInfo>();
		recentDurasi = new HashMap<DTNHost, EncounterInfo>();
	 ...
	 */
	public SprayAndFocusRouterCoba(SprayAndFocusRouterCoba r)
	{
		super(r);
	...
	{
		return new SprayAndFocusRouterCoba(this);
	}
	...
	public void changedConnection(Connection con)
	{
		...
		if(this.recentDurasi.containsKey(peer))
		{ 
			EncounterInfo info = this.recentDurasi.get(peer);
			info.updateDurationTime(SimClock.getTime());
		}
		else
		{
			this.recentDurasi.put(peer, new EncounterInfo(SimClock.getTime()));
		}
...
//			System.out.print("mati");
			...
			this.recentDurasi.put(peer, new EncounterInfo(SimClock.getTime()-startTimestamps.get(peer)));
//			System.out.println("node f"+peer.getAddress()+" durasi "+getLastDurationForHost(peer));
			//System.out.print("Node "+peer.getAddress()+" Durasi "+(SimClock.getTime()-startTimestamps.get(peer))+"\n");
			this.Durasi.put(peer, SimClock.getTime()-startTimestamps.get(peer));
			neighborEncounters.remove(peer);
			return;
		}else{
			startTimestamps.put(peer, SimClock.getTime());
//			System.out.println(startTimestamps.get(peer));
		}
		 ...
		 */
		int msgSize = recentDurasi.size() * 64 + getMessageCollection().size() * 8;
		Message newMsg = new Message(thisHost, peer, SUMMARY_XCHG_IDPREFIX + protocolMsgIdx++, msgSize);
		newMsg.addProperty(SUMMARY_XCHG_PROP, /*new HashMap<DTNHost, EncounterInfo>(*/recentDurasi);
	...
	public Message messageTransferred(String id, DTNHost from)
	{
		...
		if(isDeliveredMessage(m) && peerEncounters != null)
		{
			...
			for(Map.Entry<DTNHost, EncounterInfo> entry : peerEncounters.entrySet())
			{
				...
				EncounterInfo info = recentDurasi.get(h);
//				System.out.print(recentDurasi.get(h));
				...
				{
					recentDurasi.get(h).updateDurationTime(peerEncounter.getDurationTime()-timediff);
				}
			}
			return m;
		}
	...
	public void update()
	{
		...
		for (Message m : getMessageCollection())
		{
			...
			else
			{
				...
				double thisLastSeen = getLastDurationForHost(dest);
//				System.out.println("node "+thisLastSeen);
				for(Connection c : getConnections())
				{
//					System.out.println("kirim ntap");
...
//						System.out.println("node "+dest.getAddress()+" durasi "+getLastDurationForHost(peer));
	...
	protected double getLastDurationForHost(DTNHost host)
	{
		if(this.recentDurasi.containsKey(host)){
//			System.out.println("dur");
			return this.recentDurasi.get(host).getDurationTime();
		}else{
			return 0.0;
		}
	}
